...
impl<'a> ShellCmd<'a> {
    ...
    pub fn new(args: &'a [String]) -> Result<Self, Errno> {
        // TODO: ensure file descriptors are created correctly in redir variants

        // find special symbols,
        // right now includes `|`, `>`, and `<`
        let mut args_iterator = args.into_iter();
        if let Some(first_pipe) = args_iterator.position(|x| x == "|") {
        ...
        } else if let Some(first_larrow) = args_iterator.position(|x| x == "<") {
        ...
        } else if let Some(first_rarrow) = args_iterator.position(|x| x == ">") {
